...
* Crear una función
1) Se toma de ejemplo el siguiente código: 
(defun my/insert-line-before ()
  ...
  (save-excursion
    ...
    (newline)))
1) dónde (interactive) dicta que el código anterior se va a tomar cómo una función
2) For evaluation press C-M-x (eval-defun)
* Definir una función
1) (global-set-key (kbd "C-7") 'undo) ; el nombre de la funcion se pone con comilla simple
* Deshacer en emacs
1) Deshacer, emacs C-/
* Iniciar una función junto con emacs
1) (require 'multiple-cursors)
* ¿Cómo agregar un directorio para cargar librerias a emacs? 
...
* Cómo ver que procesos demandan más uso en emacs 
...
* Agregar un keybinding a una secuencia con dos o más caracteres
1) Error:
   1) (global-set-key "\C-xl" 'make-symbolic-link) (da un eror de sintaxis debido a que la combinacion
      C-x ya está definida. muestra el error
   2) "Key sequence __ starts with non-prefix key _
2) Solucion:

   1) Hay que hacer un unbindig de la combinación C-x

      1) (global-set-key (kbd "M-t") nil) ;; Remove the old keybindin

      2) (global-unset-key (kbd "C-m") ) ;; o de está manera 
...
2) Hacer toogle:
   1) 2) tecla 't'
3) Seleccionar todos los directorios
   1) */
4) Abrir todos los archivos
   1) Seleccionar archivos
   2) !
      1) new window: firefox
5) Buscar texto dentro de los archivos
   1) shift+a=A
      1) Texto a buscar
6) Actualizar
   1) g
...
8) Copiar:
   1) C
9) Cambiar el orden del directorio:
   1) S
10) Deseleccionar todo
    1) U
11) Hacer un buffer editable
    1) C-x C-q
    2) Actualizar los cambios
       1) C-c C-g
    3) Refresh: g
* Linea de comandos
1) multi-term
